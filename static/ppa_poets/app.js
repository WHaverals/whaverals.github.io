// IMPORTANT: use jsDelivr `+esm` build to avoid bare imports like "events"
// that the browser cannot resolve without a bundler/import map.
import Graph from "https://cdn.jsdelivr.net/npm/graphology@0.25.4/+esm?v=20260211-graphology";
...
async function parseGexfToGraph(gexfText) {
  ...
  const defaultEdgeType = (graphEl?.getAttribute("defaultedgetype") || "undirected").toLowerCase();
  const graphType = defaultEdgeType === "directed" ? "directed" : "undirected";

  // Use multi graph to avoid failures on duplicates
  const graph = new Graph({ type: graphType, multi: true });
...
try {
  ...
  const graph = await parseGexfToGraph(gexfText);
